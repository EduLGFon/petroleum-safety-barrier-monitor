...
import {
  computeKpi, computeChartData,
  applyFilters, applySorting, paginate, defaultFilters,
} from '@/lib/utils';
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    case 'SET_LOCATION':
      return { ...state, location: action.payload, filters: { ...state.filters, page: 1 } };
    case 'SET_FILTER':
      return { ...state, filters: { ...state.filters, ...action.payload, page: 1 } };
    case 'SET_SORT': {
      const col = action.payload;
      const dir = state.filters.sortCol === col && state.filters.sortDir === 'asc' ? 'desc' : 'asc';
      return { ...state, filters: { ...state.filters, sortCol: col, sortDir: dir } };
    }
    case 'RESET_FILTERS':
...
export function useDashboard(allBarriers: Barrier[]) {
  ...
  const hasActiveFilters =
    !!state.filters.query || !!state.filters.disponibilidade ||
  ...
  return {
    ...
    kpi, chartData,
    rows,
    allFiltered:   sorted,       // full filtered+sorted set (for export)

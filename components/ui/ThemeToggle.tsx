...
export function ThemeToggle() {
  ...
  return (
    <div style={{display:'flex',background:'rgba(0,0,0,.22)',border:'1px solid rgba(255,255,255,.1)',borderRadius:10,padding:3,gap:2}}>
      {OPTS.map(({value,Icon,label})=>{
        const a=theme===value;
        return (
          <button key={value} onClick={()=>setTheme(value)} title={label} aria-pressed={a}
            className={a?'animate-theme':''}
            style={{display:'flex',alignItems:'center',gap:5,padding:'5px 10px',fontSize:11,fontWeight:600,borderRadius:7,border:'none',cursor:'pointer',
              background:a?'linear-gradient(135deg,var(--accent),var(--accent-2))':'transparent',
              color:a?'#fff':'rgba(148,163,184,.8)',
              boxShadow:a?'0 2px 6px var(--glow)':'none',
              transition:'all .22s var(--ease-std)'}}>
            <Icon size={12} color={a?'#fff':'rgba(148,163,184,.8)'} strokeWidth={2.5}/>{label}
          </button>
        );
      })}
    </div>

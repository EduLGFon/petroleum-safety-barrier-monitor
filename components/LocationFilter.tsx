      ...
      {LOCATIONS.map((loc, i) => {
        ...
        return (
          <button key={loc.code} onClick={()=>onChange(loc.code)} aria-pressed={a} title={loc.tipo}
            className="lift"
            style={{ display:'flex', alignItems:'center', gap:7, padding:'7px 14px', fontSize:12,
              fontWeight:a?700:500, borderRadius:10,
              ...
              boxShadow:a?'0 4px 14px var(--glow)':'var(--shadow-sm)',
              whiteSpace:'nowrap',
              animation:`staggerFade .35s ${i*40}ms cubic-bezier(.34,1.2,.64,1) both`,
            }}>
            {loc.name}
            <span style={{ fontSize:10, fontWeight:600, padding:'1px 6px', borderRadius:5,
              background:a?'rgba(255,255,255,.22)':'var(--bg-elevated)',

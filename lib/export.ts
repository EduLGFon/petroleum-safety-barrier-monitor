...
function lightenArgb(hex: string, f = 0.88) {
  const h = hex.replace("#", "");
  const r = Math.round(
    parseInt(h.slice(0, 2), 16) + (255 - parseInt(h.slice(0, 2), 16)) * f,
  );
  const g = Math.round(
    parseInt(h.slice(2, 4), 16) + (255 - parseInt(h.slice(2, 4), 16)) * f,
  );
  const b = Math.round(
    parseInt(h.slice(4, 6), 16) + (255 - parseInt(h.slice(4, 6), 16)) * f,
  );
  return "FF" + r.toString(16).padStart(2, "0") +
...
export async function exportToExcel(
    ...
    vals.forEach((v, ci) => {
      ...
      if (ci === 9) {
        ...
        if (cfg) {
          cell.fill = {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: lightenArgb(cfg.solid, 0.86) },
          };
          cell.font = {
            name: "Calibri",
            size: 9,
            bold: true,
            color: { argb: hexArgb(cfg.solid) },
          };
          cell.alignment = { horizontal: "center", vertical: "middle" };
        }
      }
      if (ci === 11) {
        ...
        if (cfg) {
          cell.fill = {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: lightenArgb(cfg.solid, 0.90) },
          };
          cell.font = {
            name: "Calibri",
            size: 9,
      ...
      if (ci === 7) {
        ...
        cell.font = {
          ...
          size: 9,
          bold: isCrit,
          color: { argb: isCrit ? "FFF97316" : "FF64748B" },
        };
        cell.alignment = { horizontal: "center", vertical: "middle" };
      }
      if (ci === 10 && v) {
  ...
  const kpi = [
    ...
    [
      "Disponíveis",
      barriers.filter((b) => b.disponibilidade === "Disponível").length,
    ],
    [
      "Fora de Operação",
      barriers.filter((b) => b.disponibilidade === "Fora de Operação").length,
    ],
    [
      "Ind. Contingenciado",
      barriers.filter((b) =>
        b.disponibilidade === "Indisponível Contingenciado"
      ).length,
    ],
    [
      "Degr. Contingenciado",
      barriers.filter((b) => b.disponibilidade === "Degradado Contingenciado")
        .length,
    ],
    [
      "Degradado (NC)",
      barriers.filter((b) => b.disponibilidade === "Degradado").length,
    ],
    [
      "Indisponível (NC)",
      barriers.filter((b) => b.disponibilidade === "Indisponível").length,
    ],
    ["Conformes", barriers.filter((b) => b.conformidade === "Conforme").length],
    [
      "Não Conformes",
      barriers.filter((b) => b.conformidade === "Não Conforme").length,
    ],
    [
      "% Conformidade",
      `${
...
export async function exportToPDF(
  ...
  doc.text(
    `${ts()}  |  ${barriers.length.toLocaleString("pt-BR")} registros`,
    14,
    25,
  );
  ...
  autoTable(doc, {
    ...
    body: barriers.map((b) => [
      ...
      b.criticidade,
      b.disponibilidade,
      b.conformidade === "Não Conforme" && b.statusSince
    ...
    didParseCell(d) {
      ...
      if (d.column.index === 5) {
        const [r, g, b] = hr(
          DISP_COLORS[String(d.cell.raw)]?.solid ?? "#94a3b8",
        );
      ...
      if (d.column.index === 7) {
        const [r, g, b] = hr(
          CONF_COLORS[String(d.cell.raw)]?.solid ?? "#94a3b8",
        );
        d.cell.styles.textColor = [r, g, b];
        d.cell.styles.fontStyle = "bold";
      }
      if (d.column.index === 6 && String(d.cell.raw) !== "—") {
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    doc.setTextColor(148, 163, 184);
    doc.text(
      `Seacrest Petróleo · Monitor de Barreiras · Página ${i} de ${pages}`,
      14,
